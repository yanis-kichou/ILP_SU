...
//***************************************** Modification *******************************************
ILP_Object
ILP_sinus(ILP_Object v){
	if ( ILP_isInteger(v) ) {
             ILP_Object result = ILP_AllocateFloat();
             result->_content.asFloat =
                  sin(v->_content.asInteger);
             return result;
        }
	if ( ILP_isFloat(v) ) {
             ILP_Object result = ILP_AllocateFloat();
             result->_content.asFloat =
                  sin(v->_content.asFloat);
             return result;
	}
	return ILP_domain_error("Not a Number", v);
}

ILP_Object
ILP_makeVector(ILP_Object o1, ILP_Object o2)
{
	if ( o1->_class == &ILP_object_Integer_class ) {
		ILP_Object result=ILP_malloc(sizeof(ILP_Object), &ILP_object_Vector_class);
		int length=o1->_content.asInteger;
		result->_content.asVector._size=length;
		result->_content.asVector.asContents=(ILP_Object *)ILP_malloc(sizeof(ILP_Object *)*length, &ILP_object_Object_class);
		int i=0;
		for (i=0;i<length;i++){
			result->_content.asVector.asContents[i]=o2;
		}
		return result;
	}
	return ILP_domain_error("Not Integer makeVector(?,value)", o1);
}

ILP_Object
ILP_vectorGet(ILP_Object o1, ILP_Object o2)
{
	if(o1->_class == &ILP_object_Vector_class && o2->_class == &ILP_object_Integer_class){
		int position=o2->_content.asInteger;
		return o1->_content.asVector.asContents[position];
	}
	return ILP_domain_error("Not Vector vecotrGET(?,?) no Vector(ILP_vector) and no Index(Integer) ", o1);
}

ILP_Object
ILP_vectorLength(ILP_Object o)
{
	if(o->_class == &ILP_object_Vector_class ){
		return ILP_Integer2ILP(o->_content.asVector._size);
	}
	return ILP_domain_error("Not Vector vectorLength(?)", o);
}
...
ILP_print (ILP_Object self)
{
     if ( self->_class == &ILP_object_Integer_class ) {
    	 ...
    	 int length =self->_content.asVector._size;
    	 fprintf(stdout,"[");
    	 int i=0;
    	 for (i=0;i<length;i++){
    		 ILP_print(self->_content.asVector.asContents[i]);
    		 if(i<length-1)
    			 fprintf(stdout,", ");
